...
export default function DashboardPage(): React.ReactNode {
  ...
  const selectedDocIds = useMemo(() => selectedIds.filter((id) => documents.some((d) => d._id === id)), [selectedIds, documents]);
    ...
    async function load(): Promise<void> {
      ...
      const parsed: PDFDocumentMeta[] = data
        .map((d) => {
          ...
          const uploadedAt = typeof obj.uploadedAt === "string" || obj.uploadedAt instanceof Date ? new Date(String(obj.uploadedAt)) : null;
      ...
      <div className={styles.main}>
        <aside className={styles.sidebar}>
          <div className={styles.sidebarInner}>
            ...
            <DocumentList documents={documents} selectedIds={selectedDocIds} onToggle={onToggle} onRemove={onRemove} />
            {docsState === "error" ? (
        ...
        <section className={styles.chat}>
          <ChatInterface sessionId={sessionId} selectedDocIds={selectedDocIds} hasDocuments={hasDocuments} />
        </section>
      </div>
    </div>
  );
